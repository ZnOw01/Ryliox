...
def is_safe_url(url: str) -> bool:
    """Check if URL is allowed for remote fetches."""
    try:
        parsed = urlparse(url)
    except ValueError:
        return False
...
def normalize_remote_url(raw_url: str, *, base_url: str = "") -> str:
    """Normalize remote URL. Returns empty string if not allowed."""
...
def normalize_asset_url(base_url: str, asset_url: str) -> str:
    """Normalize asset URL. Returns empty string if not allowed."""
...
def sanitize_remote_url(raw_url: str, *, base_url: str = "") -> str:
    """Sanitize remote URL. Returns empty string if not allowed."""
    return normalize_remote_url(raw_url, base_url=base_url)


def ensure_safe_asset_url(url: str) -> None:
    """Raise ValueError if asset URL is not safe."""
    try:
