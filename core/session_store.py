...
def _normalize_cookie_record(
    raw: Mapping[str, Any], *, default_domain: str | None = None
) -> CookieRecord | None:
    name = _normalize_cookie_name(raw.get("name"))
    if not name:
        return None

    domain_value = str(raw.get("domain") or default_domain or "").strip().lower()
    # Preserve leading dot for wildcard domains (e.g., .example.com)
...
class SessionStore:
    ...
    def _connect(self) -> sqlite3.Connection:
        """Establish a SQLite connection with WAL mode."""
    ...
    def _read_legacy_json(self) -> list[CookieRecord]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Failed to read legacy cookies file with UTF-8 BOM fallback (%s): %s",
                    path,
                    fallback_exc,
                )
                return []
    ...
    @staticmethod
    def _read_legacy_sqlite(path: Path) -> list[CookieRecord]:
        """Read cookies from a legacy SQLite file with a ``session_cookies`` table.

        Returns an empty list if the file is not a database, is unreadable, or
        ...
        for name, value in rows:
            record = _normalize_cookie_record(
    ...
    def _read_legacy_table_in_current_db(self) -> list[CookieRecord]:
        """Read cookies from a ``session_cookies`` table inside ``self.db_path``."""
        try:
    ...
    def save_cookies(self, payload: Any) -> int:
        ...
        Existing cookies not present in ``payload`` are deleted. The result
        is exactly the set described by ``payload`` (deduplicated and
        normalised).
        """
    ...
    def get_cookie_records(self) -> list[CookieRecord]:
        """Load non-expired cookie records from the database."""
        ...
        with self._lock:
            try:
                ...
                logger.warning("Failed to load cookie records from database: %s", exc)
                return []
    ...
    def load_cookie_records(self, migrate_legacy: bool = True) -> list[CookieRecord]:
        """Load cookie records, with optional migration from legacy stores.

        Migration sources, in order:
          1. A ``session_cookies`` table in the same database (legacy v1 schema)
          2. A ``session_cookies`` table in the file at ``legacy_cookies_file``
             (a previous SQLite store at a different path)
          3. A JSON / cookie-header file at ``legacy_cookies_file``
        """
        cookies = self.get_cookie_records()

        if self._legacy_migration_attempted or not migrate_legacy:
            return cookies
        ...
        legacy_records: list[CookieRecord] = []

        # 1. Same-DB legacy table
        try:
            legacy_records = self._read_legacy_table_in_current_db()
        except sqlite3.Error as exc:
            logger.warning("Failed to read legacy table in current DB: %s", exc)

        # 2. External legacy SQLite file
        if not legacy_records and self.legacy_cookies_file.exists():
            from_sqlite = self._read_legacy_sqlite(self.legacy_cookies_file)
            if from_sqlite:
                legacy_records = from_sqlite
            else:
                # 3. JSON / cookie-header file
                legacy_records = self._read_legacy_json()

        if not legacy_records:
            return cookies

        try:
            self.save_cookies(legacy_records)
        except sqlite3.Error as exc:
            logger.warning(
                "Failed to migrate legacy cookies from %s: %s",
                self.legacy_cookies_file,
                exc,
            )
    ...
    def has_cookies(self, allow_legacy_fallback: bool = True) -> bool:
        if self.count_stored_cookies() > 0:
            return True
        if not allow_legacy_fallback:
            return False

        return bool(self._read_legacy_json())
