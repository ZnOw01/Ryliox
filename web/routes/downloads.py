...
async def cleanup_old_files_task(
    ...
    try:
        ...
        if not output_dir.exists():
            logger.debug("Output directory does not exist, skipping cleanup: %s", output_dir)
            return
...
async def progress_stream(
    ...
    async def event_stream():
        ...
        try:
            ...
            logger.exception("[%s] Error in SSE stream for job_id=%s: %s", request_id, job_id, exc)
            raise
        finally:
...
def download(
    ...
    if not data.book_id:
        raise HTTPException(
            ...
            detail={"error": "Book ID is required", "code": ErrorCode.BOOK_ID_REQUIRED},
        )
    ...
    if data.output_dir:
        ...
        if not success:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error": str(message), "code": ErrorCode.INVALID_OUTPUT_DIR},
            )
    else:
    ...
    if selected_chapters is not None:
        unsupported = sorted(
            fmt for fmt in formats if not DownloaderPlugin.supports_chapter_selection(fmt)
        )
        if unsupported:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
