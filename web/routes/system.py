...
def health(
    ...
    try:
        ...
        # Verify by attempting to count cookies
        cookie_count = session_store.count_stored_cookies()
    ...
    if status != "ok":
        ...
        logger.warning(f"Health check status '{status}' - failed components: {failed_details}")
...
async def health_detailed(
    ...
    # Determine overall status
    failed_checks = [name for name, check in checks.items() if check.get("status") == "error"]
    degraded_checks = [name for name, check in checks.items() if check.get("status") == "degraded"]
...
async def reveal_file(
    ...
    if not data.path:
        raise HTTPException(
            ...
            detail={"error": "Path is required", "code": ErrorCode.PATH_REQUIRED},
        )
        ...
        if selected:
            ...
            if not success:
                return error_response(
                    message or "Invalid output directory",
...
async def get_feature_flags_api() -> FeatureFlagsResponse:
    ...
    """
    return FeatureFlagsResponse(flags=feature_flags.get_all(), config=feature_flags.get_config())
...
async def get_single_feature_flag(flag_name: str) -> SingleFeatureFlagResponse:
    ...
    """
    return SingleFeatureFlagResponse(flag=flag_name, enabled=feature_flags.is_enabled(flag_name))
