...
def error_response(
    ...
    if not (400 <= status_code < 600):
        raise ValueError(f"error_response requiere un status 4xx/5xx, recibido: {status_code}")
...
def not_found_response(message: str, code: ErrorCode | str = ErrorCode.NOT_FOUND) -> JSONResponse:
    """Atajo para 404."""
