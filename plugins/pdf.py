...
class PdfPlugin(Plugin):
    ...
    def _prepare_windows_gtk_path(self):
        """Make GTK runtime DLLs visible to WeasyPrint on Windows.

        The GTK installer sometimes does not update PATH for already-open shells,
        or installs to a standard location that Python's DLL loader won't search.
        """
        if sys.platform != "win32":
            return

        # Avoid noisy GLib/GIO UWP warnings printed by GTK on Windows.
        os.environ.setdefault("GIO_USE_VFS", "local")
        os.environ.setdefault("GIO_USE_VOLUME_MONITOR", "unix")

        candidates = [
            os.environ.get("GTK3_RUNTIME_DIR"),
        ...
        for candidate in candidates:
            if not candidate:
                continue
            dll_path = Path(candidate) / "libgobject-2.0-0.dll"
            ...
            if hasattr(os, "add_dll_directory"):
                ...
                self._dll_directory_handles.append(os.add_dll_directory(candidate))
            ...
            try:
                ...
                # Let WeasyPrint/cffi raise the detailed dependency error below.
                pass
            return
    ...
    def weasyprint(self):
        ...
        if self._weasyprint is None:
            try:
                self._prepare_windows_gtk_path()
                import weasyprint

                self._weasyprint = weasyprint
            except (ImportError, OSError) as e:
                error_text = str(e)
                if "cffLib" in error_text and "fontTools" in error_text:
                    raise ImportError(
                        "PDF generation failed because the local fontTools install is broken or incompatible. "
                        "This commonly happens with the Python 3.14 Windows venv. Stop the server, remove `.venv`, "
                        "then run `uv sync` so uv selects Python >=3.11,<3.14."
                    ) from e
                raise ImportError(
                    "WeasyPrint is required for PDF generation. "
                    "Install with: pip install weasyprint\n"
                    "System dependencies (Windows): install GTK3 runtime from "
                    "https://github.com/tschoonj/GTK-for-Windows-Runtime-Environment-Installer/releases "
                    "then restart your terminal. If it is already installed, run "
                    "`where libgobject-2.0-0.dll` and ensure the containing `bin` folder is in PATH.\n"
...
<body>
    ...
    def _build_combined_html(
        ...
        for chapter in chapters:
            ...
            chapter_class = "chapter chapter-first" if not chapters_html_parts else "chapter"

            chapters_html_parts.append(f'''
    <section class="{chapter_class}" id="{chapter_id}">
        {body}
    </section>''')
...
<body>
    ...
    def _generate_cover_html(
        ...
        cover_img = ""
        if cover_image:
            cover_img = f'<img src="Images/{html.escape(cover_image, quote=True)}" alt="Cover">'
    ...
    def _generate_toc_html(self, toc: list[dict], chapters: list[dict]) -> str:
        ...
        def render_item(item: dict) -> str:
            ...
            if item.get("children"):
                children_items = "".join(
                    f"<li>{render_item(child)}</li>" for child in item["children"]
                )
    ...
    def _load_css_files(self, oebps: Path, css_files: list[str]) -> str:
        ...
        for i, _ in enumerate(css_files):
            ...
            if css_path.exists():
                ...
                css_text = self._resolve_css_urls(css_text, css_path, oebps)
                css_parts.append(css_text)

        return "\n".join(css_parts)

    def _resolve_css_urls(self, css_text: str, css_file_path: Path, base_dir: Path) -> str:
        ...
        def replacer(m):
            ...
            if (
                ...
                return m.group(0)
            abs_path = (css_dir / url).resolve()
            try:
                rel_path = abs_path.relative_to(base_dir.resolve())
            except ValueError:
                return m.group(0)
            return f'url("{rel_path.as_posix()}")'
