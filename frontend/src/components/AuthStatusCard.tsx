...
export function AuthStatusCard() {
	...
	const shouldShowCookieEditor =
		showCookieEditor || (!checkingSession && !sessionHealthy);
	...
	}, [cookiesQuery.data?.cookies, cookiesText]);
	...
	const formatCookiesJson = useCallback(() => {
		try {
			const parsed = JSON.parse(cookiesText);
			setCookiesText(JSON.stringify(parsed, null, 2));
		} catch {
			// ignore invalid JSON
		}
	}, [cookiesText]);
	...
	useEffect(() => {
		...
		const targetHeight = Math.min(
			maxHeight,
			Math.max(minHeight, el.scrollHeight + 2),
		);
		...
		<section className="soft-rise min-w-0 flex-shrink-0 self-start overflow-hidden rounded-2xl border border-border bg-card p-5 shadow-panel backdrop-blur-sm">
			...
			{shouldShowCookieEditor ? (
				<div id="cookie-editor">
					...
					{cookiesText.trim() ? (
						<FormattedJsonPreview json={cookiesText} />
					) : null}
