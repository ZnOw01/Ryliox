...
export function SearchBooksCard() {
  ...
  const filteredResults = useMemo(() => {
    ...
    const filtered = results.filter(book => {
      ...
      return id.includes(value) || title.includes(value) || authors.includes(value) || publishers.includes(value);
    });
          ...
          {parsedError ? (
            ...
            >
              ...
              {parsedError.suggestion ? (
                <p className="mt-1 text-xs text-destructive-foreground/80">{parsedError.suggestion}</p>
              ) : null}
            </div>
          ) : null}
          {visibleResults.length > 0 ? (
