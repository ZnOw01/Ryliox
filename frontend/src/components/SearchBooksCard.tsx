...
function parseDirectBookRef(query: string): SearchBook | null {
	...
	const urlMatch = trimmed.match(
		/learning\.oreilly\.com\/library\/view\/([^/]+)\/(\d{10,13})/,
	);
	if (urlMatch) {
...
export function SearchBooksCard() {
	...
	const selectedBookId = useBookStore(
		(state) => state.selectedBook?.id ?? null,
	);
	...
	const directRef = useMemo(
		() => parseDirectBookRef(normalizedQuery),
		[normalizedQuery],
	);
	...
	const parsedError =
		debouncedQuery && searchQuery.error
			? parseApiError(searchQuery.error)
			: null;
	const isSearching =
	...
	useEffect(() => {
		...
		setActiveResultIndex((current) =>
			Math.min(current, visibleResults.length - 1),
		);
						...
						>
							<StaggeredLayoutContainer>
								{visibleResults.map((book, index) => {
									...
									const isActive =
										visibleResults[activeResultIndex]?.id === book.id;
									return (
										<StaggeredLayoutItem
											key={book.id}
										...
										>
											<motion.button
												...
												data-result-index={visibleResults.findIndex(
													(item) => item.id === book.id,
												)}
												role="option"
											...
											>
												<div className="flex min-w-0 items-start justify-between gap-2 sm:gap-3">
													...
													>
														{isSelected
															? t("search.book.selected")
															: t("search.book.select")}
													</Badge>
												</div>
											</motion.button>
										</StaggeredLayoutItem>
