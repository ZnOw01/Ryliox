...
export function useDownloadManager() {
	...
	const progressQueryKey = useMemo(
		() => queryKeys.progress.byJob(activeJobId),
		[activeJobId],
	);
	...
	useEffect(() => {
		...
		const validIndexes = new Set(
			chaptersQuery.data.chapters.map((chapter) => chapter.index),
		);
	...
	const invalidFormatWithChapterSelection =
		hasChapterSelection && bookOnlyFormats.has(format);
	const selectedFormatDescription = format
		? formatsQuery.data?.descriptions?.[format]
		: undefined;
	...
	const startMutation = useMutation({
		...
		onMutate: async (): Promise<{
			...
			const previousProgress =
				queryClient.getQueryData<ProgressResponse>(progressQueryKey);
	...
	const chaptersLoading =
		Boolean(selectedBook) && !chaptersQuery.data && chaptersQuery.isLoading;
			...
			: invalidFormatWithChapterSelection
				...
				: active
					...
					: startMutation.isPending
						...
						: null;
	const startDisabledReason = shouldShowStartHint
		? startDisabledReasonBase
		: null;
